    ...
    <Paper variant="outlined" sx={{ p: 2, textAlign: 'center', maxWidth: qrSize + 50 }}>
      <Box sx={{ mb: 1 }}>
        <Typography variant="subtitle2" color="text.secondary">
          {detectedField.type === 'signature' ? t('entViz.signatureQr') : t('entViz.verificationQr')}
        </Typography>
      </Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1, mb: 1 }}>
        <IconButton size="small" onClick={handleCopy} title={copied ? t('entViz.copied') : t('entViz.copyData')}>
          <ContentCopy fontSize="small" />
        </IconButton>
        <IconButton size="small" onClick={handleDownload} title={t('entViz.downloadQr')}>
