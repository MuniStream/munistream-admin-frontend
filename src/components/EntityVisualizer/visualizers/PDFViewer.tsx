  ...
  useEffect(() => {
    const fetchPDF = async () => {
      try {
        ...
        setError(t('entViz.pdfLoadFailed'));
      } finally {
        setLoading(false);
      }
    };
    ...
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        <Typography variant="subtitle2" color="text.secondary">
          {t('entViz.pdfDocument')}
        </Typography>
        <Box>
          <IconButton onClick={handleOpen} title={t('entViz.openPdf')} size="small">
            <OpenInNew />
          </IconButton>
          <IconButton onClick={handleDownload} title={t('entViz.downloadPdf')} size="small">
            <GetApp />
          </IconButton>
        </Box>
      </Box>
      ...
      {numPages > 1 && (
        <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1, textAlign: 'center' }}>
          {t('entViz.pageOf', { current: 1, total: numPages })}
        </Typography>
      )}
    </Paper>
  );
