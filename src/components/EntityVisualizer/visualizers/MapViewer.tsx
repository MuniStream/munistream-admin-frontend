  ...
  return (
    <Paper variant="outlined" sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
        ...
        <Chip label={t('entViz.geocoded')} size="small" variant="outlined" color="primary" />
      </Box>
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <Typography variant="caption" color="text.secondary">
          {t('entViz.clickToViewMap')}
        </Typography>
      </Box>

      <Box sx={{ display: 'flex', gap: 1 }}>
        <IconButton size="small" onClick={handleOpenInMaps} title={t('entViz.openInGoogleMaps')}>
          <OpenInNew fontSize="small" />
        </IconButton>
        <IconButton size="small" onClick={handleCopy} title={copied ? t('entViz.copied') : t('entViz.copyAddress')}>
      ...
      {options.showCoordinates && options.coordinates && (
        <Box sx={{ mt: 2, pt: 2, borderTop: 1, borderColor: 'divider' }}>
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block' }}>
            {t('entViz.coordinates', { lat: options.coordinates.lat, lng: options.coordinates.lng })}
          </Typography>
        </Box>
      )}
    </Paper>
