...
export const DigitalSignatureForm: React.FC<DigitalSignatureFormProps> = ({
  ...
  const pkcs1ToPkcs8 = (pkcs1Key: Uint8Array): Uint8Array => {
    // PKCS#8 wrapper for RSA private key
    // This is the ASN.1 structure for PKCS#8 RSA private key
    const rsaOid = new Uint8Array([
      0x30, 0x0D,  // SEQUENCE (13 bytes)
      0x06, 0x09,  // OBJECT IDENTIFIER (9 bytes) - RSA encryption
    ...
    // Calculate length encoding sizes
    const getLengthEncoding = (length: number): number[] => {
      if (length < 0x80) {
        return [length];
    ...
    const keyLengthEncoding = getLengthEncoding(keyLength);
    const versionBytes = 3; // INTEGER tag + length + value
    const algorithmBytes = rsaOid.length;
    const octetStringHeaderBytes = 1 + keyLengthEncoding.length; // tag + length encoding
    const contentLength = versionBytes + algorithmBytes + octetStringHeaderBytes + keyLength;
    const outerLengthEncoding = getLengthEncoding(contentLength);
    const totalLength = 1 + outerLengthEncoding.length + contentLength; // SEQUENCE tag + length + content

    console.log('🔧 PKCS#8 size calculation:', {
      keyLength,
      versionBytes,
      algorithmBytes,
      octetStringHeaderBytes,
      contentLength,
      totalLength
    });

    // Create PKCS#8 structure with correct size
    let pkcs8 = new Uint8Array(totalLength);
    let offset = 0;

    // Outer SEQUENCE
    pkcs8[offset++] = 0x30; // SEQUENCE tag
    for (const byte of outerLengthEncoding) {
      pkcs8[offset++] = byte;
    }
    ...
    pkcs8[offset++] = 0x00; // value

    // Algorithm identifier
    pkcs8.set(rsaOid, offset);
    offset += rsaOid.length;

    // Private key (OCTET STRING containing the PKCS#1 key)
    pkcs8[offset++] = 0x04; // OCTET STRING tag
    for (const byte of keyLengthEncoding) {
      pkcs8[offset++] = byte;
    }

    pkcs8.set(pkcs1Key, offset);

    console.log('🔧 Converted PKCS#1 to PKCS#8, original:', pkcs1Key.length, 'converted:', pkcs8.length);
    return pkcs8;
  };

  const parseCertificateBasicInfo = (certPem: string) => {
    // Basic certificate info parsing - for display purposes
    // Real validation will be done on the backend
    try {
      const certData = pemToBinary(certPem);
      return {
